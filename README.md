...
* The EEGsynth is intended to be run from the Linux command line, using python and bash scripts, and is not friendly for those not familiar with such an approach
* The codebase and technical documentation are maintained on our [GitHub repository](https://github.com/eegsynth/eegsynth)
* It is strongly adviced to work within you own cloned repository and keep up to date with EEGsynth commits, as the EEGsynth is in constant development
* You can follow us on [Facebook](https://www.facebook.com/EEGsynth/) and [Twitter](https://twitter.com/eegsynth), read our [news blog](http://www.ouunpo.com/eegsynth/?page_id=621), and check our past and upcoming events on [our calendar](http://www.ouunpo.com/eegsynth/?calendar=eegsynth-calendar)
...
* Please feel free to contact us with que on collaborations via our [contact form](http://www.ouunpo.com/eegsynth/?page_id=233)
* For installation please follow our installation instructions for [Linux](https://github.com/eegsynth/eegsynth/blob/master/doc/installation-linux.md), [OSX](https://github.com/eegsynth/eegsynth/blob/master/doc/installation-osx.md) and [Windows](https://github.com/eegsynth/eegsynth/blob/master/doc/installation-windows.md). Please note that OSX and Windows are not activily supported, as Linux is the intended operating system, partly because we would like to keep the EEGsynth compatible with the Raspberry-Pi, partly because some Python libraries do not support Windows, and partly because we rely on command-line interface anyway.
...
The spectral module calculates the spectral power in different frequency bands. Those frequency bands, and their name, are given in the .ini file. As you can see some are defined by numbers ('alpha=9-11'), while others use Redis keys ('redband=plotsignal.redband.lo-plotsignal.redband.hi'). In the latter case, the frequency band is determined (via Redis) by the plotsignal module, which can be used to visually select frequency bands. In its turn, the spectral module outputs (power) values to Redis, prefixed by 'spectral' (see its output field). As you can see, the .ini file includes other settings as well. You can find a default .ini in their respective directory, with a filename identical to the module. E.g. [www.github.com/eegsynth/eegsynth/modules/spectral/](www.github.com/eegsynth/eegsynth/modules/spectral/) contains [spectral.py](www.github.com/eegsynth/eegsynth/modules/spectral/spectral.py) and [spectral.ini](www.github.com/eegsynth/eegsynth/modules/spectral/spectral.ini). Initialization files can be edited with any text editor but should be saved in a separate 'patch directory', in which you store all the .ini files belonging to one patch. This helps organizing your patches as well as your local git repository, which will then not create conflicts with the remote default .ini files.
...
## Controlling external software & hardware
