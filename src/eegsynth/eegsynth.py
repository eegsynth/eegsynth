...
# This is the EEGsynth application. It starts all modules in a patch where each module
# corresponds to an ini file specified on the command-line. The ini files must start with
...
try:
    # this works when the package is installed, but not when it is run from the source directory
    __version__ = metadata.version('eegsynth')
except:
    # this works when the package is run from the source directory
    with open(path + '/../../pyproject.toml', 'r') as f:
        pyproject = toml.load(f)
    __version__ = pyproject['project']['version']

# the lib and module directory are located relative to the eegsynth executable
sys.path.insert(0, os.path.join(path, '..'))

# import libraries, these are located relative to the eegsynth executable
from lib import EEGsynth
from lib import FieldTrip

# import modules, these are located relative to the eegsynth executable
# some modules are excluded because of too complex dependencies
